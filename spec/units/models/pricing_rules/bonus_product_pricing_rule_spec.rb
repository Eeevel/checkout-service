...
RSpec.describe BonusProductPricingRule do
  describe '#initialize' do
    ...
    context 'with valid arguments' do
      it 'initializes with required arguments' do
        expect { described_class.new(required_product_code, required_count) }.not_to raise_error
      end
    end
    ...
    context 'with invalid arguments' do
      ...
      it 'raises an error if required_count is not an integer' do
        expect do
          described_class.new(required_product_code,
                              'invalid')
  ...
  describe '#calculate' do
    ...
    context 'when products meet the required count' do
      ...
      it 'applies the bonus product pricing rule' do
        pricing_rule = described_class.new('GR1', 3)
        expect(pricing_rule.calculate(products)).to eq(Money.from_cents(200, 'GBP')) # (3 / 2 + 3 % 2) * 100 = 200
      end
    end
