...
RSpec.describe PercentagePricingRuleValidator do
  describe '.validate_initializer' do
    context 'with valid arguments' do
      it 'does not raise an error' do
        expect { described_class.validate_initializer('GR1', 3, 10) }.not_to raise_error
      end
    end
    ...
    context 'with invalid arguments' do
      ...
      it 'raises an error if percentage is nil' do
        expect do
          described_class.validate_initializer('GR1', 3,
                                               nil)
        end.to raise_error(ValidationError, 'Validation failed: Percentage must be a positive or zero number')
